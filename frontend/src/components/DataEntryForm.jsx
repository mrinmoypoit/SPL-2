...
const DataEntryForm = ({ product, onSaved, onCancel }) => {
    ...
    useEffect(() => {
        if (product) {
            ...
            if (product.subcategory_id) {
                setSelectedCategory('savings_accounts'); // Default, could be improved
            }
        }
    }, [product]);
    ...
    const fetchCompanies = async () => {
        try {
            ...
            // Note: You may need to create a dedicated endpoint for companies
            if (response.ok) {
                // Temporary: use from products response
                setCompanies([]);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = product 
                ? `${apiBaseUrl}/admin/products/${product.product_id}`
